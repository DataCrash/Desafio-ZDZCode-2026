...
SYNC_STATUS_SCRIPT="$ROOT_DIR/scripts/github/sync_board_status.sh"
...
if [[ ! -x "$SYNC_STATUS_SCRIPT" ]]; then
  echo "Erro: script não executável ou inexistente: $SYNC_STATUS_SCRIPT"
  exit 1
fi
...
echo "==> Board Ops: sincronizando status dos cards"
GH_OWNER="$GH_OWNER" GH_REPO="$GH_REPO" BOARD_TITLE="$BOARD_TITLE" "$SYNC_STATUS_SCRIPT"
